...
namespace ShooterGuys.Screens
{
    class PlayerSelectScreen : GameScreen
    {
        ...
        private Tank.ControlScheme[] _selectedControls = new Tank.ControlScheme[4];

        private int _playersPlaying = 0;

        private Tank[] _tankSprites = new Tank[4];
        ...
        public override void LoadContent()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                _tankSprites[i] = new Tank(i, new Vector2(180 + (i * 300), 300), 0, Tank.ControlScheme.Empty, new Rectangle(0, 0, (int)GameSettings.GetResolution().X, (int)GameSettings.GetResolution().Y));
        ...
        private void ChangeTeam(int spaces, int playerIndex)
        {
            ...
            {
                newTeam += spaces;
                if (newTeam < 0) { newTeam = 3; }
                else if (newTeam > 3) { newTeam = 0; }
                foundNewTeam = true;
                for (int j = 0; j < 4; j++)
                {
                    if (playerIndex != j)
        ...
        public override void CustomDraw(Microsoft.Xna.Framework.GameTime gameTime)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                _spriteBatch.DrawString(_font, "Player " + (i + 1), new Vector2(180 + (i * 300), 200), Color.Black);
                if (_selectedControls[i] != Tank.ControlScheme.Empty)
                {
                    _spriteBatch.DrawString(_font, _selectedControls[i].ToString(), new Vector2(180 + (i * 300), 250), Color.Black);
                    _spriteBatch.DrawString(_font, "Change color:Left/Right", new Vector2(110 + (i * 300), 450), Color.Black);
                    _tankSprites[_selectedTeams[i]].position = new Vector2(180 + (i * 300), 300);
